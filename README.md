...
### 部署在云服务器上无法访问 Web 界面（5000 端口被防火墙拦截）

默认云厂商（如 GCP）的安全组 / 防火墙仅放行 22 / 80 / 443，不会自动开放 5000。直接访问 `http://<公网IP>:5000` 会超时。两种解决方式：

1. **在云控制台的安全组里放行 5000 端口**（入站 TCP 5000，来源按需限制）。
2. **不改防火墙，用 SSH 本地端口转发**（更安全，推荐）：

   ```bash
   ssh -N -L 5000:127.0.0.1:5000 -o ServerAliveInterval=30 user@<服务器公网IP>
   ```

   然后在本地浏览器打开 `http://localhost:5000` 即可。
