...
#: 源文本是日文时，译文里出现多少个**平假名**就足以断定「模型返回的还是日文」。
#:
#: 只认平假名，**不认片假名也不认谚文**：
#:
#: - 片假名借词（アニメ / トヨタ）在中文译文里是正常保留，计入会把正常译文判成
#:   未译（上一轮 Blocker-1 的假阳性就是这个形态）；
#: - 谚文不需要额外判据：纯谚文译文里根本没有汉字，非中文占比天然是 1.0，既有的
#:   比值分支已经能抓住；反过来「中文译文里留了两个韩文词」会被本判据误判。
#:
#: 而平假名在中文译文里几乎不出现，日文的助词 / 助动词（は・が・を・に・です・
#: ます）与活用又必然把它们带出来 —— 覆盖的正是重合度判据够不着的一类模型失误：
#: **没有翻译，而是把日文原文改写/清洗了一遍**（换同义词、改时态、去填充词、
#: 换汉字写法）。这类输出既不是逐字照抄（走不到 ``d == s`` 快速路径），与源的
#: 重合度又落在 0.6–0.79，只靠 ``_CJK_ECHO_RATIO_THRESHOLD`` 会整体逃逸：每行
#: 日文都是改写型返回时整批验收会放行，日文原文被当成中文译文写盘并烧录。
#:
#: **代价（复审第六轮实测口径，别按「只是那一行回退原文」来理解）**：
#:
#: - 误杀面：复审给的 10 条语料里 7 条被判未译 —— 5 条由本判据造成
#:   （`この猫はかわいいですね → 这只猫真かわいい`、`がんばれ、みんな → 大家がんばれ`、
#:   `この子はかわいい → 这个角色太「かわいい」了`、`かわいいし、すごい → 又かわいい又すごい`、
#:   `ふわふわしてる → 毛茸茸的、ふわふわ`），另 2 条是既有规则（`すごい演技だった →
#:   演技真すごい` 走重合度、`She is so cute → 她真かわいい` 走假名占比）。
#:   把本判据关掉后同一批语料只剩 2 条被误杀。
#: - 文件级后果：残留 <= 3 条且 <= 15% 时只是「该行标记后回退原文」；一旦超过
#:   （实测 20 行里 4 行 = 20%），``_finalize_residual_untranslated_items`` 直接
#:   返回 False ⇒ **整份译文被丢弃**。而 ja→zh 场景下的「回退原文」本身就是把
#:   日文原文写进中文字幕，与本次修复要消灭的形态同源，并不是无害的回退。
#:   这两条后果由用例
#:   ``SourceLanguageParaphraseReturnTests.test_hiragana_loanword_lines_can_discard_the_whole_translation``
#:   钉住（4/20 → 整批失败；1/20 → 放行并回退原文）。
#:
#: **不要为了减少误杀去改判据**：两条「看起来更精确」的替代规则实测都会重新打开
#: 漏检 —— 「平假名**连续段** >= 2」在 22 条日文改写语料上漏检 2 条
#: （`そのとおりですね` / `問題ないです` 这类整句平假名只有一段），
#: 「段数 >= 2 **或**译文无汉字」漏检 1 条（`問題ありません → 問題ないです`），
#: 而误杀只从 6/9 降到 2/9。取舍方向（宁可误杀，不放过整类日文）在这一侧是对的；
#: 真要调阈值，先在真实批次上统计「中文译文保留平假名借词」的出现频率再定。
_HIRAGANA_RETURN_MIN_COUNT = 2
#: 用于上面的计数：平假名（不含片假名 / 谚文）。
...
def _cjk_echo_ratio(source: str, target: str) -> float:
    ...
    - 比例低：译文是独立生成的（哪怕保留了「アニメ」这类假名借词），其中的
      ...
      误判的后果是把正常译文回退成原文，甚至整份译文被丢弃。注意比例低
      只说明「不是复读」：模型把日文改写/清洗一遍的失误由调用方的平假名
      计数兜底（见 ``_HIRAGANA_RETURN_MIN_COUNT``）。
    """
...
class SubtitleTranslator:
    ...
    def _likely_untranslated(self, src: str, dst: str) -> bool:
        ...
        源本身是日文时，「译文里的汉字算不算中文」由两个信号裁决：与源的重合度
        （复读）或译文里的平假名个数（模型把日文改写/清洗了一遍）。详见
        ``_HIRAGANA_RETURN_MIN_COUNT`` 与下面的场景注释。
        """
        ...
        try:
            ...
            # 中文」需要区分两种场景：
            #   1) 源文本本身就是日文 / 韩文（含假名 / 谚文）：此时看两个信号 ——
            #      译文与源的**重合度**（基本照搬源文本 = 复读），以及译文里
            #      **平假名**的个数（模型没翻译、只把日文改写/清洗了一遍）。
            #      不能用「源里有假名」这个全局信号：它会把「中文译文里保留了一个
            #      片假名借词」（アニメ / トヨタ）整句判成未译，后果是该条被回退成
            #      原文、甚至整份译文被判「未译残留」丢弃 —— 与本次修复的目标形态
            #      相反。片假名因此不计入计数（见 _HIRAGANA_RETURN_MIN_COUNT）。
            ...
            if dst_has_foreign_cjk:
                ...
                if src_has_foreign_cjk:
                    # 源是日文时两个信号并联，「照抄」与「改写型返回」都要覆盖：
                    #   - 重合度高（>= 0.8）：基本逐字复读源文本；
                    #   - 译文含 >= 2 个平假名：模型没翻译，只把源文改写/清洗了
                    #     一遍（重合度落在 0.6–0.79，单靠上面那条会整体漏检）。
                    # 只数平假名，所以「中文译文保留片假名借词」不会被误判
                    # （**平假名**借词的误杀与文件级后果见
                    # _HIRAGANA_RETURN_MIN_COUNT 的代价说明）；
                    # 韩文走既有的比值分支（纯谚文译文没有汉字，占比天然 1.0）。
                    cjk_is_foreign = (
                        _cjk_echo_ratio(s, d) >= _CJK_ECHO_RATIO_THRESHOLD
                        or len(_HIRAGANA_RE.findall(d)) >= _HIRAGANA_RETURN_MIN_COUNT
                    )
                else:
