...
# 以下两张表只用于声明"哪些键需要做整数 / 浮点归一化"（消费方仅取 keys）。
# 注意：它们携带的数值 **不是** 设置页保存失败时的回退值来源，也未必与
# DEFAULT_CONFIG 相同（例如 SUBTITLE_MAX_LINE_LENGTH 在此为 999、在
# DEFAULT_CONFIG 中为 42）。回退值一律由 config_manager.DEFAULT_CONFIG 提供
# （例外见 app.py 的 _PINNED_SETTINGS_DEFAULTS），改这里不会影响回退行为。
...
# 同 SPEECH_PIPELINE_INT_FIELDS：仅声明键集合，数值不参与回退。
