...
_SMART_SEGMENT_WORD_BUILTIN_BEHAVIOR = (
    ...
    "5. 【核心】目标时长 2-4 秒。优先在句号、问号、感叹号等句末标点后切分；逗号后仅在相邻词组合不足以凑足 2 秒时才切。"
    "   - 当一个语意单元短于 2 秒时，必须与紧随其后的语意单元合并成一条，直到合并后时长接近 2-4 秒区间。"
    "   - 排比、并列短语（如「住天际的房子，吃天际的美食，做天际的工作」）应合并为一条，不要每个短语单独成段。"
    "   - 严禁仅按逗号边界切分导致短句堆积；每条都应是一个完整的呼吸节奏单元。"
    "6. 单条文本保持简洁，避免过长换行（建议单行不超过 42 个可见字符）。"
)
...
_SMART_SEGMENT_SEGMENT_BUILTIN_BEHAVIOR = (
    ...
    "4. 节奏约束：每条时长不少于 {min_duration_s} 秒、不超过 {max_duration_s} 秒；可见字符速率不超过 {max_cps} 字/秒。"
    "5. 【核心】目标时长 2-4 秒。优先在句末标点处切分；短段必须与相邻短段合并直到接近 2-4 秒区间，禁止仅按逗号边界切分。"
    "   - 排比、并列短语应合并为一条，不要每个短语单独成段。"
)
