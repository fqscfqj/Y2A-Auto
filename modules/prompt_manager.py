...
_SMART_SEGMENT_WORD_BUILTIN_BEHAVIOR = (
    ...
    "\n"
    "## 断句优先级（从高到低）\n"
    "1. 【句末标点】句号 . 、问号 ? 、感叹号 ! 、句号 。 之后 —— 最理想的切分点。\n"
    "2. 【分句标点】分号 ; 、冒号 : 、分号 ； 之后。\n"
    "3. 【从句/并列连接词之前】and、but、or、that、which、who、where、when、if、because、so、like、although、while 等之前。\n"
    "4. 【逗号/顿号处】中文逗号 ， 、顿号 、 是非常好的次级切分点；英文逗号 , 后如有明显语意转折也可切分。\n"
    "5. 【介词短语开始处】before、after、with、without、for、from 等引导新短语时。\n"
    "\n"
    "## 绝对禁止的切分（红线）\n"
    ...
    "- 禁止让任何一条字幕以孤立的介词/连词/助词结尾（如 from、and、in、to、的、了、在、是）\n"
    "- 禁止在中文主语和谓语之间切分（如 我 | 认为、他 | 说）\n"
    "- 禁止把「把」字句/「被」字句的处置对象和动作拆开\n"
    "- 禁止在关联词的前后两半之间切分（虽然…但是、因为…所以、不但…而且）\n"
    "- 排比、列举、并列短语（如 house, food, and job）→ 合并为一条，不要逐项拆散。\n"
    "\n"
    "## 节奏建议\n"
    "- 建议每条时长 2-6 秒，优先保证语义完整而非精确掐时间。\n"
    "\n"
    "## 技术约束\n"
...
_SMART_SEGMENT_SEGMENT_BUILTIN_BEHAVIOR = (
    ...
    "1. 句末标点（. ! ? 。）之后 —— 最优先。\n"
    "2. 分句标点（; : ；）之后。\n"
    "3. 从句/并列连接词之前（and、but、or、that、which、where、when、if、because、so、although）。\n"
    "4. 逗号/顿号处（， 、）—— 中文逗号是非常好的次级切分点。\n"
    "\n"
    "## 绝对禁止\n"
    "- 禁止让任何一条字幕以孤立的介词/连词/助词结尾（的、了、在、是、and、of、in）。\n"
    "- 禁止在人名/地名/专有名词中间切分。\n"
    "- 禁止在冠词-名词、形容词-名词、介词-宾语之间切分。\n"
    "- 禁止在中文主语和谓语之间切分。\n"
    "- 禁止把「把」字句/「被」字句的处置对象和动作拆开。\n"
    "- 禁止在关联词的前后两半之间切分（虽然…但是、因为…所以）。\n"
    "\n"
    "## 节奏建议\n"
    "- 建议每条时长 2-6 秒，优先保证语义完整而非精确掐时间。\n"
    "\n"
